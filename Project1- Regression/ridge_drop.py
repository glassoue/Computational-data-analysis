...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 24 14:50:30 2020
...
for i in range(100):
    for j in range(101):
        if df.iloc[i,j]==' NaN':
            ind_nan_cat=ind_nan_cat+1
            list_nan.append(i)

list_nan=np.asarray(list_nan)   
list_nan=np.unique(list_nan)         
df=df.drop(list_nan)
...
# here NaN obs are dropped
[n, p] = np.shape(X)

lambda_values = np.logspace(-10, 10, 1000)
